...
const startNewRound = async (roomId) => { 
    ...
    if (!drawerUser) {
        io.to(roomId).emit('chatMessage', { user: 'System', message: 'Not enough active players for a new round. Game ending.' });
        endGame(roomId);
        return;
    }

    room.players.forEach(player => {
        player.isDrawer = (player.id === room.drawerId); 
        io.to(player.id).emit('disableDrawing'); 
    });
    ...
    io.to(roomId).emit('chatMessage', { user: 'System', message: `Round ${room.currentRound}/${room.maxRounds} begins! ${drawerUser.username} is drawing.` }); 
...
io.on('connection', (socket) => { 
    ...
    if (!rooms.has(GAME_ROOM_ID)) { 
        rooms.set(GAME_ROOM_ID, { 
            ...
            gameStarted: false, 
            wordHint: '',
            timeLeft: ROUND_TIME // Initialize timeLeft here
        }); 
    } 
    ...
    socket.on('joinGame', ({ username }) => {  
        if (!username || username.trim() === '') { 
            ...
            io.to(socket.id).emit('chatMessage', { user: 'System', message: 'No username entered, joining as ' + username }); 
        } 
        ...
        if (currentRoom.gameStarted) { 
            ...
            if (currentRoom.roundTimer) { 
                io.to(socket.id).emit('updateTimer', Math.ceil(currentRoom.timeLeft)); 
            } 
        } else { 
        ...
        if (user) { 
            ...
            if (socket.id === currentRoom.drawerId && currentRoom.gameStarted) { 
                ...
                if (currentRoom.players.size > 0) {
                    ...
                    endGame(GAME_ROOM_ID); // End game if no players left
                }
            } 
